...
import { ChevronDown, Search, X, Heart, BookOpen, Users, Handshake, Award } from "lucide-react";
    ...
    <>
      ...
      >
        <div className="max-w-[1200px] mx-auto px-4 md:px-[32px] w-full flex items-center justify-between h-[64px]">
                  ...
                  <AnimatePresence>
                    {dropdownOpen && (
                      <motion.div
                        ...
                        className="absolute top-[52px] left-1/2 -translate-x-1/2 min-w-[320px] bg-white rounded-[16px] shadow-[0_12px_48px_rgba(0,0,0,0.12)] border border-[var(--border-color)] mt-2 p-3"
                      >
                        <div className="flex flex-col gap-0.5">
                          {link.children.map((child) => (
                            <Link
                              key={child.path}
                              to={child.path}
                              className="flex items-center gap-3 px-4 py-3 rounded-[10px] hover:bg-[var(--teal-light)] transition-colors group"
                            >
          ...
          {/* Mobile Hamburger */}
          <div className="lg:hidden flex items-center justify-center w-[40px] h-[40px] cursor-pointer z-[9999]" onClick={() => setMobileOpen(!mobileOpen)} aria-label={mobileOpen ? "Close menu" : "Open menu"}>
            <div className="relative w-[20px] h-[14px]">
              <span className={`absolute left-0 w-[20px] h-[2px] rounded-[2px] transition-all duration-[280ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${burgerBg}`} style={{ top: "0px", transform: mobileOpen ? "translateY(6px) rotate(45deg)" : "none", backgroundColor: mobileOpen ? "var(--dark)" : undefined }} />
              <span className={`absolute left-0 top-[6px] w-[20px] h-[2px] rounded-[2px] transition-all duration-[280ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${burgerBg}`} style={{ opacity: mobileOpen ? 0 : 1, transform: mobileOpen ? "scaleX(0)" : "scaleX(1)", backgroundColor: mobileOpen ? "var(--dark)" : undefined }} />
              <span className={`absolute left-0 w-[20px] h-[2px] rounded-[2px] transition-all duration-[280ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${burgerBg}`} style={{ top: "12px", transform: mobileOpen ? "translateY(-6px) rotate(-45deg)" : "none", backgroundColor: mobileOpen ? "var(--dark)" : undefined }} />
            </div>
          </div>
        </div>
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ y: "-100%" }} animate={{ y: "0%" }} exit={{ y: "-100%" }} transition={{ duration: 0.25, ease: "easeOut" }} className="global-card absolute top-full left-0 right-0 shadow-[var(--shadow-lg)] z-40 border-0 rounded-none border-b border-[var(--border-color)]">
              <form onSubmit={handleSearch} className="max-w-[800px] mx-auto relative flex items-center">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search articles, initiatives..." autoFocus className="w-full text-[16px] text-[var(--dark)] font-normal border-b-2 border-[var(--border-color)] focus:border-[var(--teal)] transition-colors py-3 pl-8 pr-8 outline-none bg-transparent no-float" style={{ fontFamily: 'var(--font)' }} />
                <button type="button" onClick={() => setSearchOpen(false)} className="absolute right-0 text-[var(--light)] hover:text-[var(--dark)] transition-colors"><X size={20} /></button>
              </form>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 32, stiffness: 280 }} className="fixed inset-y-0 right-0 w-[100vw] bg-white z-[9998] flex flex-col overflow-y-auto" style={{ top: "36px" }}>
              <div className="flex flex-col gap-6 px-6 pt-12 pb-8">
                {navLinks.map((link, i) => (
                  <motion.div key={link.path || link.label} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.055, duration: 0.3, ease: [0.25, 0.46, 0.45, 0.94] }}>
                    {link.children ? (
                      <div className="flex flex-col gap-3">
                        <span className="text-[11px] font-semibold text-[var(--dark)] uppercase tracking-wider">{link.label}</span>
                        {link.children.map((child) => (
                          <Link key={child.path} to={child.path} onClick={() => setMobileOpen(false)} className="flex items-center gap-3 text-[15px] font-medium text-[var(--mid)] hover:text-[var(--teal)] transition-colors pl-2 py-1">
                            <child.icon size={16} className="text-[var(--teal)]" />
                            {child.label}
                          </Link>
                        ))}
                      </div>
                    ) : (
                      <Link to={link.path} onClick={() => setMobileOpen(false)} className="text-[16px] font-semibold text-[var(--dark)] hover:text-[var(--teal)] transition-colors block">{link.label}</Link>
                    )}
                  </motion.div>
                ))}
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: navLinks.length * 0.055, duration: 0.3 }} className="mt-8">
                  <button onClick={() => { setMobileOpen(false); openOverlay(); }} className="w-full bg-[var(--yellow)] text-[var(--dark)] font-semibold text-[15px] h-[48px] rounded-full border-none flex items-center justify-center shadow-[var(--shadow-sm)] active:scale-95 transition-transform">
                    {t("nav.donate")}
                  </button>
                </motion.div>
              </div>
