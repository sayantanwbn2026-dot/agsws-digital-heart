      ...
      <div className="max-w-[var(--container)] mx-auto px-6 relative z-10" ref={inViewRef}>
        <FadeInUp className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-white/[0.06] text-white/60 text-[11px] font-[600] uppercase tracking-[0.12em] px-4 py-1.5 rounded-full border border-white/[0.08] mb-4">
            <Activity size={12} /> Live Impact Analytics
          </div>
          <h2 className="text-[28px] lg:text-[36px] font-[800] text-white tracking-[-0.02em]">Your Donations at Work</h2>
          <p className="text-white/40 text-[14px] mt-2 max-w-[500px] mx-auto">Real-time visualization of how every rupee creates impact across Kolkata.</p>
        </FadeInUp>
          ...
          <div className="space-y-5">
            <FadeInUp delay={0.1}>
              <div className="bg-white/[0.04] backdrop-blur-sm rounded-[20px] border border-white/[0.06] p-6">
                <div className="flex items-center gap-4 mb-4">
                  <MiniDonut value={78} max={100} color="var(--teal)" />
                  <div>
                    <p className="text-[24px] font-[800] text-white">{inView ? <CountUp end={78} duration={2} /> : 0}%</p>
                    <p className="text-[11px] text-white/40 uppercase tracking-[0.06em] font-[500]">Direct to Programs</p>
                  </div>
                </div>
                <p className="text-[12px] text-white/30 leading-[1.6]">78p of every rupee goes directly to medical and education programs.</p>
              </div>
            </FadeInUp>
            ...
            <FadeInUp delay={0.15}>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { icon: Heart, label: "Patients", value: 2400, color: "var(--teal)" },
                  { icon: GraduationCap, label: "Students", value: 850, color: "var(--purple)" },
                  { icon: Users, label: "Families", value: 1200, color: "var(--yellow)" },
                  { icon: TrendingUp, label: "Success %", value: 94, color: "#16A34A" },
                ].map((s, i) => (
