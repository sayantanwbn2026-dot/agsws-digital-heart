  ...
  return (
    <section ref={sectionRef} className="relative min-h-[100svh] flex items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: contentY, opacity: opacityOut }} className="relative z-10 text-center max-w-[860px] mx-auto px-6 py-32 will-change-transform">
        ...
        <h1 className="display-hero text-white mb-6 max-w-[800px] mx-auto">
          ...
          {["at", "a", "Time."].map((word, i) => (
            <motion.span
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.8 + i * 0.07, duration: 0.45 }}
              className="inline-block mr-3"
            >
              {word}
            </motion.span>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.5 }} className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 mb-12">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="px-9 py-4 text-[14px] font-[700] bg-[var(--yellow)] text-[var(--dark)] rounded-full shadow-[0_4px_16px_rgba(242,183,5,0.25)] hover:shadow-[0_8px_32px_rgba(242,183,5,0.4)] transition-shadow flex items-center justify-center gap-2"
          >
        ...
        <motion.div
          ...
          className="flex flex-wrap justify-center divide-x divide-white/[0.1] mb-8"
        >
        ...
        {/* Trust badges */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.5 }} className="flex flex-wrap justify-center gap-5 text-white/45">
          {[
            ...
            <span key={text} className="flex items-center gap-1.5 text-[11px] font-[500]">
              <Icon size={12} className="text-[var(--yellow)]/80" />
              {text}
            </span>
          ))}
        </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-28 left-6 z-20 hidden lg:block"
      >
