    ...
    <main id="main-content">
      <section className="h-[300px] bg-gradient-to-br from-teal-dark via-teal to-teal flex items-center justify-center relative">
        ...
        <div className="relative z-10 text-center">
          <h1 className="heading-1 text-primary-foreground">Volunteer Portal</h1>
          <p className="text-sm text-primary-foreground/60 mt-3">Track your hours and download certificates</p>
        </div>
      </section>
      ...
      <section className="bg-background py-16">
        <div className="max-w-[480px] mx-auto px-6">
          ...
          <p className="text-xs text-text-light mt-2">Try: VOL-2025-0042 or VOL-2024-0018</p>
        </div>
          ...
          {searched && error && (
            <motion.div key="err" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-[560px] mx-auto mt-8 px-6">
              <div className="bg-yellow-light border border-yellow/30 rounded-xl p-6 flex gap-3">
                ...
                <p className="text-sm text-text-mid">Volunteer ID not found. Check your welcome email for the correct ID.</p>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="res" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-[560px] mx-auto mt-8 px-6">
              <div className="global-card">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <p className="font-bold text-xl text-text-dark">{result.name}</p>
                    <span className="text-xs font-semibold bg-teal-light text-teal px-2 py-0.5 rounded-full">{result.role}</span>
                    <p className="text-xs text-text-light mt-1">Member since {result.since} · {volId.toUpperCase()}</p>
                  </div>
                </div>

                <div className="bg-teal-light rounded-xl p-5 mb-6">
                  <p className="text-4xl font-extrabold text-teal">{result.totalHours} <span className="text-lg font-medium">hours</span></p>
                  <p className="text-sm text-text-mid">contributed to AGSWS</p>
                  <div className="flex h-2 rounded-full overflow-hidden mt-3 gap-0.5">
                    {Object.entries(result.categories).map(([key, val]: any) => {
                      const colors: Record<string, string> = { field: "bg-teal", medical: "bg-teal-dark", education: "bg-purple", admin: "bg-beige" };
                    ...
                    {Object.entries(result.categories).map(([key, val]: any) => {
                      ...
                      return (
                        <div key={key} className="flex items-center gap-1">
                          ...
                          <span className="text-[10px] text-text-light capitalize">{key} ({val}h)</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <h4 className="font-semibold text-sm text-text-dark mb-3">Recent Activities</h4>
                <div className="space-y-3 mb-6">
                  {result.activities.map((a: any, i: number) => (
                    <div key={i} className="flex gap-3 text-sm">
                      <span className="text-xs text-teal font-medium w-20 flex-shrink-0">{a.date}</span>
                      <span className="text-text-mid flex-1">{a.desc}</span>
                      <span className="text-xs text-text-light flex-shrink-0">{a.hours}h</span>
                    </div>
                  ))}
                </div>

                <div className="bg-yellow-light rounded-xl p-5">
                  <div className="flex items-center gap-2 mb-2">
                    <Award size={20} className="text-yellow" />
                    <span className="font-semibold text-sm text-text-dark">Request a Volunteer Certificate</span>
                  </div>
                  <p className="text-xs text-text-mid mb-3">Download a certificate confirming your volunteer service with AGSWS.</p>
                  <button onClick={() => alert("Certificate generation will be available on launch.")} className="w-full bg-yellow text-text-dark font-bold py-3 rounded-lg flex items-center justify-center gap-2 text-sm">
