    ...
    <main id="main-content">
      <section ref={heroRef} className="relative h-[380px] md:h-[440px] overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-6">
          <motion.span initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="inline-block text-[10px] font-semibold uppercase tracking-[0.2em] text-[hsl(var(--accent))] mb-4">From The Field</motion.span>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.6 }} className="text-4xl md:text-5xl font-extrabold text-white tracking-tight">Impact Stories</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="text-sm text-white/50 mt-4 max-w-md mx-auto">Real stories of transformation from the streets of Kolkata</motion.p>
        </div>
      </section>
              ...
              <div className="relative rounded-2xl overflow-hidden bg-[hsl(var(--card))] border border-[hsl(var(--border))] shadow-sm hover:shadow-lg transition-shadow duration-500">
                <div className="grid grid-cols-1 lg:grid-cols-2">
                  <div className="relative h-[280px] lg:h-[420px] overflow-hidden">
                    ...
                    <span className={`absolute top-5 left-5 text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider ${categoryColors[featured.category] || "bg-[hsl(var(--accent))]/10 text-[hsl(var(--foreground))]"}`}>{featured.category}</span>
                  </div>
                  <div className="p-8 lg:p-12 flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl lg:text-3xl font-bold text-[hsl(var(--foreground))] mb-4 group-hover:text-[hsl(var(--primary))] transition-colors duration-300 leading-tight">{featured.title}</h2>
                    <p className="text-sm text-[hsl(var(--muted-foreground))] leading-relaxed mb-6 line-clamp-3">{featured.excerpt}</p>
                    <span className="inline-flex items-center gap-2 text-[hsl(var(--primary))] text-sm font-semibold group-hover:gap-3 transition-all duration-300">Read Full Story <ArrowRight size={14} /></span>
                  </div>
                </div>
              </div>
            </Link>
                ...
                <Link to={`/blog/${story.slug}`} className="group block h-full">
                  <motion.div whileHover={{ y: -6 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="rounded-2xl overflow-hidden bg-[hsl(var(--card))] border border-[hsl(var(--border))] h-full flex flex-col shadow-sm">
                    <div className="relative h-[200px] overflow-hidden">
                      ...
                      <span className={`absolute top-4 left-4 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider z-10 ${categoryColors[story.category] || "bg-white/90 text-[hsl(var(--foreground))]"}`}>{story.category}</span>
                    </div>
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <h4 className="text-base font-bold text-[hsl(var(--foreground))] mb-2 line-clamp-2 group-hover:text-[hsl(var(--primary))] transition-colors duration-300">{story.title}</h4>
                      <p className="text-[13px] text-[hsl(var(--muted-foreground))] line-clamp-3 leading-relaxed flex-1">{story.excerpt}</p>
                      <span className="inline-flex items-center gap-1.5 mt-5 text-[hsl(var(--primary))] text-[13px] font-semibold group-hover:gap-2.5 transition-all duration-300">Read Story <ArrowRight size={12} /></span>
                    </div>
                  </motion.div>
                </Link>
              </FadeInUp>
