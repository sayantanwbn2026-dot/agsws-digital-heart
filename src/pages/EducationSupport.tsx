            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-auto">
              <div className="global-card w-full max-w-[520px] my-8">
                ...
                <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setShowApply(false); }}>
                  <input placeholder="Child's Full Name *" required className="global-card w-full h-12 outline-none focus: text-sm" />
                  <div className="grid grid-cols-2 gap-3">
                    <input placeholder="Child's Age *" required type="number" className="global-card w-full h-12 outline-none focus: text-sm" />
                    <input placeholder="Class / Grade *" required className="global-card w-full h-12 outline-none focus: text-sm" />
                  </div>
                  <input placeholder="School Name & Address *" required className="global-card w-full h-12 outline-none focus: text-sm" />
                  <input placeholder="Parent/Guardian Name *" required className="global-card w-full h-12 outline-none focus: text-sm" />
                  <input placeholder="Parent/Guardian Phone *" required type="tel" className="global-card w-full h-12 outline-none focus: text-sm" />
                  <input placeholder="Parent/Guardian Email *" required type="email" className="global-card w-full h-12 outline-none focus: text-sm" />
                  <textarea placeholder="Brief reason for applying" rows={3} maxLength={300} className="global-card w-full outline-none focus: text-sm" />
