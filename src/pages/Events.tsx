    ...
    >
      ...
      <div className="p-6">
        <div className="flex items-start gap-5">
          <div className={`flex-shrink-0 w-16 h-16 rounded-2xl bg-gradient-to-br ${typeGradients[event.type] || typeGradients.medical} flex flex-col items-center justify-center shadow-[0_4px_12px_rgba(0,0,0,0.1)]`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <span className={`text-[10px] font-[600] px-2.5 py-1 rounded-full uppercase tracking-[0.04em] ${eventTypeColors[event.type] || 'bg-teal text-primary-foreground'}`}>
                {eventTypeLabels[event.type] || event.type}
              </span>
              {!event.isPast && spotsLeft < 20 && (
                <span className="text-[10px] font-[600] px-2 py-0.5 rounded-full bg-red-50 text-red-600">{spotsLeft} spots left</span>
              )}
            </div>
        ...
        {!event.isPast && (
          <div className="mt-4">
            ...
            <div className="h-1.5 bg-[var(--bg)] rounded-full overflow-hidden">
              <motion.div className={`h-full rounded-full bg-gradient-to-r ${typeGradients[event.type] || typeGradients.medical}`} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.3, ease: "easeOut" }} />
            </div>
          </div>
        )}
        <div className="flex items-center gap-3 mt-5">
          ...
          <button onClick={onClick} className="text-[13px] font-[600] text-[var(--teal)] hover:underline transition-colors">Details</button>
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="bg-white rounded-[20px] border border-[var(--border-color)] shadow-[var(--shadow-card)] p-6">
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {["S","M","T","W","T","F","S"].map((d, i) => (<span key={i} className="text-[10px] font-[600] text-[var(--light)] py-1.5 uppercase">{d}</span>))}
...
const Events = () => {
  ...
  return (
    <main id="main-content">
      <PageHero title="Events & Campaigns" label="Community" subtitle="Medical camps, school programmes, and community drives — happening near you in Kolkata." size="md" bgVariant="teal" />
        ...
        <div className="max-w-[var(--container)] mx-auto px-[var(--container-px)] py-5 flex flex-wrap justify-center gap-8">
          {[
            ...
            <div key={label} className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-[var(--teal-light)] flex items-center justify-center"><Icon size={16} className="text-[var(--teal)]" /></div>
              <div>
        ...
        <div className="max-w-[var(--container)] mx-auto px-[var(--container-px)] grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
          <div>
            ...
            <div className="space-y-4">
              {upcoming.map((evt, i) => (<EventCard key={evt.id} event={evt} onClick={() => setSelectedEvent(evt)} index={i} />))}
              {upcoming.length === 0 && (
                <div className="text-center py-16"><Calendar size={40} className="text-[var(--border-color)] mx-auto mb-4" /><p className="text-[var(--mid)] text-[14px]">No upcoming events in this category.</p></div>
              )}
            </div>
            {past.length > 0 && (
              <div className="mt-10">
